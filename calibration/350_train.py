...
def get_train_loader(dataset,  batch):
    ...
    if dataset == "cifar10":
        ...
        train_loader = torch.utils.data.DataLoader(
            traindata, batch_size=batch, sampler=train_sampler,
            num_workers=1, pin_memory=True,
        )
        valid_loader = torch.utils.data.DataLoader(
            valdata, batch_size=batch, sampler=valid_sampler,
            num_workers=1, pin_memory=True,
        )
...
modelurl = url+'idx'+'+epoch'+str(ep_num)+'-r50step777.model'
df = pd.DataFrame(zip(ep_list, val_acc, test_acc))

df.to_csv(url+'r50step777.csv')
torch.save(best_model_dict, modelurl)
ender.record()


model1 = resnet.resnet50()
dim_in = model1.fc.in_features
model1.fc =nn.Linear(in_features=dim_in, out_features=10, bias=True)
model1 = model1.cuda(0)
model1.load_state_dict(modelurl)
model1.eval()

with torch.no_grad():
    info=[]
    logits, labels = get_logits_labels2(testloader, model1)
    softmaxes = F.softmax(logits, dim=1)
    _, predictions = torch.max(softmaxes, 1)
