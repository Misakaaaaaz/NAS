...
#scheduler = CosineAnnealingLR(optimizer, T_max=(epochs+35)*len(trainloader))
warmups=20*len(trainloader)
#bsize = len(trainloader)

#try1
#try2:eta = 0.005
lamda = lambda curiter: curiter/warmups if curiter<warmups else \
    (0.01+0.5*0.9*(1.0+math.cos((curiter-warmups)/(epochs*len(trainloader))*math.pi)))
...
trys='try3'
print(trys)
modelurl = url+'idx'+str(idx)+'+epoch'+str(ep_num)+'-450coswarmup-'+trys+'.model'
df = pd.DataFrame(zip(ep_list, val_acc, test_acc))

df.to_csv(url+str(idx)+'450coswarmup-'+trys+'.csv')
